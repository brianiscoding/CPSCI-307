...
temp_getFilesOnly = []
temp_allFilesToSave = []
temp_getFilesHashesText = []
temp_getFilesOnlyNameOnly = []
for x1, x2, x3, x4 in zip(
    getFilesOnly, allFilesToSave, getFilesHashesText, getFilesOnlyNameOnly
):
    if f"Group{id_group}" not in x1:
        continue
    id_subset = int(x1.split("_")[-1].split(".")[0])
    ...
    temp_getFilesOnly.append(x1)
    temp_allFilesToSave.append(x2)
    temp_getFilesHashesText.append(x3)
    temp_getFilesOnlyNameOnly.append(x4)

getFilesOnly = temp_getFilesOnly
allFilesToSave = temp_allFilesToSave
getFilesHashesText = temp_getFilesHashesText
getFilesOnlyNameOnly = temp_getFilesOnlyNameOnly
# MODIFIED END
